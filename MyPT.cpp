...
// Number of counts between branches.
UINT64 count = 0;
...
// Did we just access memory?
bool accessedMem = false;

// Last memory access
char prevMemAddress;

// Do we count this address?
bool toCount = true;
...
VOID writeMemRead(ADDRINT insAddress, ADDRINT memAddress)
{
    //std::cout << "mr" << std::endl;
    // get the string of memAddr and keep only the last 4 digits
    std::string memAddr (StringFromAddrint(memAddress));
    std::string subAddr (memAddr.substr(memAddr.length() - 4));
    //if (!accessedMem || subAddr[0] != prevMemAddress) {
        toCount = false;
        ...
        // print only if we haven't just accessed mem
        //  or if we did but we're in a new page
        std::cout << "mra:" << subAddr << "<-" << StringFromAddrint(insAddress) << std::endl;// << IMG_Name(IMG_FindByAddress(insAddress));

        // save as prev page by the digit 4th from the end
        prevMemAddress = subAddr[0];
        accessedMem = true;
    //}
}

VOID writeMemWrite(ADDRINT insAddress, ADDRINT memAddress)
{
    // get the string of memAddr and keep only the last 4 digits
    std::string memAddr (StringFromAddrint(memAddress));
    std::string subAddr (memAddr.substr(memAddr.length() - 4));
    //if (!accessedMem || subAddr[0] != prevMemAddress) {
        ...
        // print only if we haven't just accessed mem
        //  or if we did but we're in a new page
        std::cout << "mwa:" << subAddr << "<-" << StringFromAddrint(insAddress) << std::endl;
        ...
        for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
        {
            ...
            if (INS_IsMemoryRead(ins) && INS_IsStandardMemop(ins))
            {
                INS_InsertIfCall(ins, IPOINT_BEFORE, AFUNPTR(CheckNextTraceEntryPointerValid),
                                 IARG_REG_VALUE, _nextBufferEntryReg,
                                 IARG_END);
                INS_InsertThenCall(ins, IPOINT_BEFORE, AFUNPTR(writeMemRead),
            ...
            if (INS_IsMemoryWrite(ins) && INS_IsStandardMemop(ins))
            {
                INS_InsertIfCall(ins, IPOINT_BEFORE, AFUNPTR(CheckNextTraceEntryPointerValid),
                                 IARG_REG_VALUE, _nextBufferEntryReg,
                                 IARG_END);
                INS_InsertThenCall(ins, IPOINT_BEFORE, AFUNPTR(writeMemWrite),
                                   IARG_ADDRINT, INS_Address(ins),
                                   IARG_MEMORYWRITE_EA,
                                   IARG_END);
